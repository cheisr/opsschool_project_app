...
mysqlhostip = os.getenv('MYSQL_ADDRESS','localhost')
mysqluser = os.getenv('MYSQL_USER','shani')
mysqlpassword = os.getenv('MYSQL_PWD')
mysqldb = os.getenv('MYSQL_DB','crud_flask')

class Database:
    def connect(self):
        return pymysql.connect(host=mysqlhostip,user=mysqluser,password=mysqlpassword,db=mysqldb)
